...
function appendGlobals(globals, node){
    ts.forEachChild(node, node => {
        ...
        if(node.kind === ts.SyntaxKind.VariableStatement){
            ...
            appendGlobal('type', getNodeName(node), hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.ClassDeclaration) {
            appendGlobal('class', getNodeName(node), hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.InterfaceDeclaration) {
            appendGlobal('interface', getNodeName(node), hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.FunctionDeclaration) {
            appendGlobal('function', getNodeName(node), hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.ModuleDeclaration) {
            appendGlobal('module', getNodeName(node), hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.EnumDeclaration) {
            appendGlobal('enum', getNodeName(node), hasExportModifier(node));
        } else {
