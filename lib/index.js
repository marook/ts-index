...
let log = require('simple-node-logger').createSimpleLogger('ts-index.log');
...
async function addFile(filePath){
    ...
    for(let g of fileGlobals){
        console.log(`("+" "${filePath}" "${g.type}" "${g.name}" ${g.exported ? 't' : 'nil'})`);
    }
}
...
function appendGlobals(globals, node){
    ts.forEachChild(node, node => {
        ...
        if(node.kind === ts.SyntaxKind.VariableStatement){
                ...
                if(node.kind === ts.SyntaxKind.VariableDeclarationList){
                    node.declarations.forEach(d => {
                        if(d.kind === ts.SyntaxKind.VariableDeclaration){
                            appendGlobal('variable', d.name.escapedText, exported);
                        }
                        else {
            ...
            appendGlobal('type', node.name.escapedText, hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.ClassDeclaration) {
            appendGlobal('class', node.name.escapedText, hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.InterfaceDeclaration) {
            appendGlobal('interface', node.name.escapedText, hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.FunctionDeclaration) {
            appendGlobal('function', node.name.escapedText, hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.ModuleDeclaration) {
            appendGlobal('module', node.name.escapedText, hasExportModifier(node));
        } else if (node.kind === ts.SyntaxKind.EnumDeclaration) {
            appendGlobal('enum', node.name.escapedText, hasExportModifier(node));
        } else {
            throw new Error(`Unknown SyntaxKind with value ${node.kind}`);
        }
    });

    function appendGlobal(type, name, exported=false){
        globals.push({
            type,
            name,
            exported,
