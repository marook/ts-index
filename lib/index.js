...
function isPathIgnored(p){
    for(let name of ['node_modules', 'dist']){
        if(p.lastIndexOf(`${name}${path.sep}`) !== -1){
            return true;
        }
        if(p.indexOf(`${path.sep}${name}${path.sep}`) !== -1){
            return true;
        }
    }
    return false;
...
function appendGlobals(globals, node){
    ts.forEachChild(node, node => {
        ...
        if(node.kind === ts.SyntaxKind.VariableStatement){
                ...
                if(node.kind === ts.SyntaxKind.VariableDeclarationList){
                    node.declarations.forEach(d => {
                        if(d.kind === ts.SyntaxKind.VariableDeclaration){
                            ...
                            handleUnknownSyntaxKind('VariableDeclarationList', node);
                        }
                    });
                } else if (node.kind === ts.SyntaxKind.ExportKeyword || node.kind === sk.DeclareKeyword) {
                } else {
                    handleUnknownSyntaxKind('VariableStatement', node);
                }
            });
            ...
            handleUnknownSyntaxKind('top-level', node);
        }
...
function handleUnknownSyntaxKind(location, node){
    throw new Error(`Unknown SyntaxKind in ${location} ${node.kind}: ${JSON.stringify(node)}`);
}
